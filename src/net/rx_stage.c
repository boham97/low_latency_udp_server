...
void rx_framer_segment(rx_framer_t *f, const uint8_t *buf, size_t n, uint64_t rx_tsc,
    ...
    if (f->expected_seq >= 0 && h->first_seq != f->expected_seq) {
        if (h->first_seq > f->expected_seq) {
            st->gaps++;
            st->gap_msgs += (uint64_t)(h->first_seq - f->expected_seq);
            /* 북 상태가 이미 불완전하다 — 소비자가 알아야 복구 정책을 걸 수 있다 */
            f->pending_flags |= RX_FLAG_AFTER_GAP;
        } else {
            st->reorders++;
        }
    }
    f->expected_seq = h->first_seq + mc;
    ...
    for (uint16_t i = 0; i < mc && p + 2 <= end; ++i) {
        ...
        if ((type != DEEP_PLU_BUY && type != DEEP_PLU_SELL) || mlen < sizeof(deep_plu_t)) {
            st->filtered++;
            p += mlen;
            continue;
        }
...
void rx_stage_run(int fd, rx_queue_t *q, rx_stats_t *st) {
    ...
    rx_framer_t framer;
    rx_framer_init(&framer);

    for (;;) {
        ssize_t n = recvfrom(fd, buf, sizeof buf, 0, NULL, NULL);
        if (n < 0) {
            /* EAGAIN = idle 타임아웃 = 리플레이 종료 */
            break;
        }
        /* 커널에서 돌아온 직후 = 이 파이프라인이 데이터를 처음 만진 시점.
           이후 구간 지연은 전부 이 기준점과의 차이로 잰다 */
        rx_framer_segment(&framer, buf, (size_t)n, ll_rdtsc(), q, st);
    }
}
